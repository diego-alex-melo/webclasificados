...
export default function WhatsAppButton({ adId, clickCount, small = false }: WhatsAppButtonProps) {
  return (
    <a
    ...
    >
      ...
      {typeof clickCount === 'number' && clickCount > 0 && (
        <span className={`rounded-full bg-black/20 font-semibold text-white/80 ${small ? 'px-1.5 py-0.5 text-[10px]' : 'px-2 py-0.5 text-xs'}`}>
